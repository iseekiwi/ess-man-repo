...
class Fishing(commands.Cog):
    ...
    # Fishing logic
    async def do_fishing(self, ctx, message):
        """Execute the fishing minigame and process results"""
        try:
            self.logger.debug(f"Starting fishing process for {ctx.author.name}")
            
            user_data = await self.config.user(ctx.author).all()
            if not user_data.get("equipped_bait"):
                return False, "🚫 You need to equip bait first!"
    
            bait = user_data.get("bait", {})
            equipped_bait = user_data["equipped_bait"]
            
            if not bait.get(equipped_bait, 0):
                return False, "🚫 You need bait to fish!"
            ...
            await message.edit(content=f"🎣 Quick! Type **{keyword}** to catch the fish!")
            ...
            try:
                ...
                return False, f"⏰ {ctx.author.name}, you took too long! The fish got away!"
            ...
            if catch:
                ...
                return False, f"🎣 {ctx.author.name} went fishing but didn't catch anything this time."
                    
        except Exception as e:
            self.logger.error(f"Error in fishing process: {e}", exc_info=True)
            return False, "❌ An error occurred while fishing. Please try again."
